...
size_t count_added(const Template& container, const std::map<std::string, SpecialToken>& special_tokens) {
  size_t count = 0;
...
Piece parse_piece(const json& j) {
    if (j.is_string()) {
        ...
        if (s.rfind("$", 0) == 0) {
            ...
            if (rest == "" || rest == "A" || rest == "a") return Piece::Sequence(SequenceId::A, 0);
            if (rest == "B" || rest == "b") return Piece::Sequence(SequenceId::B, 0);
            // Try parse number
            try {
                 uint32_t type_id = std::stoul(rest);
            ...
            // Split by :
             ...
             if (colon != std::string::npos) {
                 ...
                 SequenceId seq = (id_part == "B" || id_part == "b") ? SequenceId::B : SequenceId::A;
                 uint32_t type_id = 0;
                 try { type_id = std::stoul(type_part); } catch(...) {}
        ...
        if (j.contains("Sequence")) {
            ...
            uint32_t type_id = s.value("type_id", 0);
            return Piece::Sequence(id_str == "B" ? SequenceId::B : SequenceId::A, type_id);
...
Template parse_template(const json& j) {
    ...
    if (j.is_array()) {
        ...
        while ((pos = s.find(delimiter)) != std::string::npos) {
            ...
            if (!token.empty()) t.push_back(parse_piece(token));
            s.erase(0, pos + delimiter.length());
        }
        if (!s.empty()) t.push_back(parse_piece(s));
    }
    return t;
}
...
std::map<std::string, SpecialToken> parse_special_tokens(const json& j) {
    ...
    for (auto it = j.begin(); it != j.end(); ++it) {
        ...
        if (val.contains("ids")) st.ids = val["ids"].get<std::vector<uint32_t>>();
        if (val.contains("tokens")) st.tokens = val["tokens"].get<std::vector<std::string>>();
        map[key] = st;
    }
    return map;
}

std::pair<std::string, uint32_t> parse_pair(const json& val) {
    if (val.is_array() && val.size() >= 2) {
        return {val[0].get<std::string>(), val[1].get<uint32_t>()};
    }
    return {"", 0};
};
...
PostProcessorConfig::PostProcessorConfig(std::string type) : type(std::move(type)) {
...
PostProcessor::Ptr PostProcessorConfig::create() const {
    ...
    TK_LOG(Error, "Unsupported PostProcessor type: %s", type.c_str());
    return std::make_shared<Sequence>(std::vector<PostProcessor::Ptr>{});
}
...
PostProcessorConfig& PostProcessorConfig::parse_json(const json& j) {
    ...
    if (type == "TemplateProcessing") {
        single = j.contains("single") ? parse_template(j["single"]) : parse_template("$0");
        pair = j.contains("pair") ? parse_template(j["pair"]) : parse_template("$A:0 $B:1");
        if (j.contains("special_tokens")) {
            special_tokens = parse_special_tokens(j["special_tokens"]);
        }
    } else if (type == "BertProcessing") {
        if (j.contains("sep")) sep = parse_pair(j["sep"]);
        if (j.contains("cls")) cls = parse_pair(j["cls"]);
    } else if (type == "RobertaProcessing") {
        if (j.contains("sep")) sep = parse_pair(j["sep"]); else sep = {"</s>", 2};
        if (j.contains("cls")) cls = parse_pair(j["cls"]); else cls = {"<s>", 0};
...
std::vector<uint64_t> TemplateProcessing::apply_template(
    ...
    // Reserve estimation
    result.reserve(tokens_a.size() + (tokens_b ? tokens_b->size() : 0) + tmpl.size());
        ...
        if (piece.is_special_token) {
            if (add_special_tokens) {
                ...
                if (it != special_tokens_.end()) {
                    ...
                    TK_LOG(Error, "TemplateProcessing: Special token '%s' not found", piece.id.c_str());
                }
            }
        } else {
            // Sequence
...
std::vector<uint64_t> BertProcessing::process(
      const std::vector<uint64_t>& tokens,
      bool /*add_special_tokens*/) const {
    TK_LOG(Info, "BertProcessing is a no-op (not implemented).");
    return tokens;
}
...
std::vector<uint64_t> RobertaProcessing::process(
      const std::vector<uint64_t>& tokens,
      bool /*add_special_tokens*/) const {
    TK_LOG(Info, "RobertaProcessing is a no-op (not implemented).");
    return tokens;
}
...
std::vector<uint64_t> Sequence::process(
    ...
    std::vector<uint64_t> current = processors_[0]->process(tokens_a, tokens_b, add_special_tokens);
