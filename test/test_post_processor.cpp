...
TEST(PostProcessorTest, TemplateProcessingSingleSequence) {
  ...
  auto processor = std::make_shared<TemplateProcessing>(single_template, pair_template, special_tokens);
...
TEST(PostProcessorTest, SequenceProcessing) {
  ...
  Template t1 = {Piece::SpecialToken("[CLS]", 0), Piece::Sequence(SequenceId::A, 0)};
  std::map<std::string, SpecialToken> st1;
  st1["[CLS]"] = {"[CLS]", {101}, {"[CLS]"}};
  auto p1 = std::make_shared<TemplateProcessing>(t1, Template{}, st1);

  // Processor 2: Append 102
  Template t2 = {Piece::Sequence(SequenceId::A, 0), Piece::SpecialToken("[SEP]", 0)};
  std::map<std::string, SpecialToken> st2;
  st2["[SEP]"] = {"[SEP]", {102}, {"[SEP]"}};
  auto p2 = std::make_shared<TemplateProcessing>(t2, Template{}, st2);
  
  auto seq_processor = std::make_shared<Sequence>(std::vector<PostProcessor::Ptr>{p1, p2});
...
TEST(PostProcessorTest, BertProcessingNoop) {
    json config_json = {
        {"type", "BertProcessing"}
    };
    auto processor = PostProcessorConfig().parse_json(config_json).create();
    ...
    EXPECT_EQ(processor->process(input), input);
}

TEST(PostProcessorTest, RobertaProcessingNoop) {
    json config_json = {
...
TEST(PostProcessorTest, EmptyInput) {
  ...
  auto processor = std::make_shared<TemplateProcessing>(single_template, Template{}, special_tokens);
...
TEST(PostProcessorTest, NestedSequenceProcessing) {
  ...
  auto inner_seq = std::make_shared<Sequence>(std::vector<PostProcessor::Ptr>{p1, p2});
  
  // p3: [200, $0, 201]
  Template t3 = {Piece::SpecialToken("200", 0), Piece::Sequence(SequenceId::A, 0), Piece::SpecialToken("201", 0)};
  std::map<std::string, SpecialToken> st3;
  st3["200"] = {"200", {200}, {"200"}};
  st3["201"] = {"201", {201}, {"201"}};
  auto p3 = std::make_shared<TemplateProcessing>(t3, Template{}, st3);

  auto outer_seq = std::make_shared<Sequence>(std::vector<PostProcessor::Ptr>{inner_seq, p3});
